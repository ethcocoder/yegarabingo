...
class TestAmountMismatch(unittest.TestCase):
    """Test amount mismatch detection between user-declared and OCR-extracted amounts."""

    def test_mismatch_detected(self):
        user_amount = 100.0
        ocr_amount = 550.0
        mismatch = abs(ocr_amount - user_amount) > 0.01
    ...
    def test_ocr_amount_takes_priority_on_mismatch(self):
        user_amount = 100.0
        ocr_amount = 550.0
        if ocr_amount > 0 and user_amount > 0 and abs(ocr_amount - user_amount) > 0.01:
            final_amount = ocr_amount
        else:
            final_amount = ocr_amount if ocr_amount > 0 else user_amount
        self.assertEqual(final_amount, 550.0)

    def test_fallback_to_user_amount_when_ocr_zero(self):
        user_amount = 100.0
        ocr_amount = 0.0
        if ocr_amount > 0 and user_amount > 0 and abs(ocr_amount - user_amount) > 0.01:
            final_amount = ocr_amount
            ...
            final_amount = ocr_amount if ocr_amount > 0 else user_amount
        self.assertEqual(final_amount, 100.0)

    def test_amount_mismatch_flag_in_deposit(self):
        deposit_data = {
            'amount': 550.0,
            'userDeclaredAmount': 100.0,
            'amountMismatch': True,
            'ocr': {'amount': 550.0},
        }
        self.assertTrue(deposit_data['amountMismatch'])
        self.assertEqual(deposit_data['amount'], 550.0)
        self.assertEqual(deposit_data['userDeclaredAmount'], 100.0)
